...
| /score/monthly-stats | GET | 获取按月统计数据（累计收入、累计支出） |
| /score/trend | GET | 获取每日积分趋势（最近7天每天获得的积分） |

> **积分趋势说明**：
> - `/score/trend` 接口返回最近7天每天获得的积分（收入），用于折线图展示
> - 横坐标为日期（YYYY-MM-DD），纵坐标为当天获得的积分总数
> - `/score/monthly-stats` 接口返回按月统计的累计收入、累计支出和月末余额，用于首页和积分页的统计展示
